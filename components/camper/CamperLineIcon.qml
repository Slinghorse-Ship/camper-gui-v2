...
Canvas {
	...
	onPaint: {
		...
		if (kind === "home") {
			...
			for (let a=0;a<8;a++){const q=a*Math.PI/4;line(c,[[w*.28+Math.cos(q)*w*.15,h*.27+Math.sin(q)*h*.15],[w*.28+Math.cos(q)*w*.21,h*.27+Math.sin(q)*h*.21]])}
			...
			for (let led=0;led<4;led++){c.beginPath();c.arc(w*(.29+led*.14),h*.48,w*.035,0,Math.PI*2);c.stroke()}
			if (kind === "rearLight") {
			...
			for(let f=0;f<3;f++){const t=f*Math.PI*2/3;c.beginPath();c.moveTo(x+Math.cos(t)*w*.1,y+Math.sin(t)*h*.1);c.quadraticCurveTo(x+Math.cos(t+.45)*w*.43,y+Math.sin(t+.45)*h*.43,x+Math.cos(t+1.3)*w*.16,y+Math.sin(t+1.3)*h*.16);c.stroke()}
		} else if (kind === "pump") {
			...
			c.beginPath(); c.arc(x,y,w*.2,0,Math.PI*2); c.stroke(); c.beginPath(); c.arc(x,y,w*.07,0,Math.PI*2); c.stroke(); for(let g=0;g<8;g++){const z=g*Math.PI/4;line(c,[[x+Math.cos(z)*w*.25,y+Math.sin(z)*h*.25],[x+Math.cos(z)*w*.4,y+Math.sin(z)*h*.4]])}
		} else if (kind === "close") {
